...
async function requestOpenLibraryWork(id: string): Promise<CatalogBook> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(response.status === 404 ? 'Livro nao encontrado.' : 'Erro ao buscar livro.')
    }
...
async function requestGoogleCatalog(params: {
  search?: string
  author?: string
  category?: string
  theme?: string
...
async function requestOpenLibraryCatalog(params: {
  search?: string
  author?: string
  category?: string
  theme?: string
