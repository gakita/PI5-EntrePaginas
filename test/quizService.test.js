...
test('creates a quiz session with two generic objective questions', () => {
...
test('records answers only for known questions', () => {
  ...
  const updated = quizService._test.recordAnswer(session, {
    questionId: 'reading_mood',
    answer: 'Reflexiva',
  });

  assert.equal(updated.answers.length, 1);
  assert.equal(updated.answers[0].questionId, 'reading_mood');
  assert.equal(updated.answers[0].answer, 'Reflexiva');
